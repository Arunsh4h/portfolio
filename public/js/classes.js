...
class arunshah_Before_After {
	constructor( $obj = null ) {
        ...
        this.$el = {
            $wrap: $obj,
            $before : jQuery('<div class="arunshah-before-after-img arunshah-before-img"/>').appendTo($obj),
            $after : jQuery('<div class="arunshah-before-after-img arunshah-after-img-wrap"/>').appendTo($obj),
            $divider : jQuery('<div class="arunshah-before-after-divider">\
        ...
        this.$el.$after.children('img').wrap('<div class="arunshah-after-img"/>');
        this.$el.$after.children('.arunshah-after-img').css('background-image', 'url('+ this.$el.$wrap.attr('data-img-after') +')');
	...
	layout() {
		this.offset = this.$el.$wrap.offset().left;
		this.size = this.$el.$wrap.width();
		this.$el.$after.children('.arunshah-after-img').width( this.$el.$wrap.width() ).height( this.$el.$wrap.height() );
	}
	reset() {
		this.current = 50;
		this.target = 50;
...
const baw_axis = new arunshah_PanAxis( 10 );
...
class arunshah_Menu {
    constructor( _core = null ) {
        ...
        this.core.$dom.header.append('<span class="arunshah-active-menu-ind"></span>');
        ...
        // Create Mobile Menu
        this.mobile_menu = new arunshah_Mobile_Menu( this.$nav );
    }
    layout() {
        const _self = this;
        let active_pos = '-100%';
...
class arunshah_Mobile_Menu {
    constructor( $obj )  {
        ...
        this.$nav = jQuery('<nav class="arunshah-mobile-nav"/>').appendTo(this.$body);
        if ( this.$el.parents('header').hasClass('is-frosted') ) {
            this.$nav.addClass('is-frosted');
        }
        this.$menu = jQuery('<ul class="arunshah-mobile-menu"/>').appendTo(this.$nav);
        this.$el.children('ul').children().each(function() {
            jQuery(this).clone().appendTo(_self.$menu);
        });
        this.$overlay = jQuery('<div class="arunshah-mobile-menu-overlay"/>').appendTo(this.$body);
        ...
        // Events
        jQuery('.arunshah-mobile-menu-toggler').on('click', function() {
...
class arunshah_Infinite_List {
    constructor( $obj = null, cfg = {} )  {
        ...
        // Hover Event
        this.$el.on('mouseenter', '.arunshah-infinite-list-item', function() {
            ...
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.arunshah-infinite-list-preview').children('video')[0].play();
                $preview.children('video')[0].play();
            }
        }).on('mouseleave', '.arunshah-infinite-list-item', function() {
            _self.$el.removeClass('is-hovererd');
            let $preview = _self.$bg_wrap.children('[data-id="'+ this.getAttribute('data-id') +'"]');
            //$preview.removeClass('is-active');
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.arunshah-infinite-list-preview').children('video')[0].pause();
    ...
    init() {
        ...
        // DOM
        this.$list = this.$el.children('.arunshah-infinite-list');
        this.$bg_wrap = jQuery('<div class="arunshah-infinite-list-bgs"/>').appendTo(this.$el);
        this.$scroll = jQuery('<div class="arunshah-infinite-scroll"/>').appendTo(this.$el);
        this.$cloned_list = jQuery('<div class="arunshah-infinite-scroll-inner"/>').appendTo(this.$scroll);
    ...
    clone_items() {
        ...
        this.$list.children().each(function(i) {
            ...
            if ( _self.first_circle ) {
                let $preview = $cloned.children('.arunshah-infinite-list-preview');
                ...
                if ( 'video' === type ) {
                    ...
                    <div class="arunshahl-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
                } else {
                    $preview.css('background-image', 'url('+ this_src + ')');
                    _self.$bg_wrap.append(`<div class="arunshahl-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`);
                }
            }
        });
...
class arunshah_Marquee {
    constructor( $obj = null )  {
        ...
        this.$inner = this.$marquee.children('.arunshah-marquee-inner');
        this.$content = this.$inner.children();
        this.width = this.$inner.find('.arunshah-marquee-list').width();
    ...
    init() {
        this.$inner.wrapInner('<div class="arunshah-marquee-inner-wrap"/>');
        this.$inner_cont = this.$inner.children('.arunshah-marquee-inner-wrap');
...
class arunshah_Masked {
    constructor( $obj = null, cfg = {} ) {
        ...
        // Options
        this.cfg = Object.assign({
            media_selector: '.arunshah-masked-media',
            content_selector: '.arunshah-masked-content',
            dbr: 24,
            ibr: 32
        }, cfg);
...
class arunshah_LWP {
    constructor( $obj = null )  {
        ...
        this.$media = jQuery('<div class="arunshah-lwp-media" data-appear="fade-right"/>').prependTo(this.$el);
        this.count = this.$list.children('.arunshah-lwp-item').length;
        this.active = 0;
        this.prev = 1;

        this.$list.children('.arunshah-lwp-item').each(function(i) {
