...
const arunshah_config = {
    ...
    // Cursor Following
    cursorFollow: '.arunshah-socials-list a, .arunshah-gallery-nav',

    // Links Exception
    linksException: [
        '.comment-reply-link',
        '.shadowcore-lightbox-link',
        '.arunshah-lightbox-link',
        '.arunshah-noanim-link'
    ]
}
...
jQuery(function ($) {
    ...
    const core = new ST_Core( arunshah_config );
    ...
    function MainMenuInit() {
        template.menu = new arunshah_Menu( core );
    }

    // --- PHOTOSWITE LIGHTBOX --- //
    function initPSWP() {
        if ( jQuery('.arunshah-lightbox-link').length ) {
            if ( typeof PhotoSwipe === 'function' ) {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close arunshah-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button arunshah-pswp-nav arunshah-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button arunshah-pswp-nav arunshah-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
                ...
                // Build PSWP Gallery
                jQuery('.arunshah-lightbox-link').each(function() {
                    ...
                    if ( $this.attr('data-type') && $this.attr( 'data-type' ) !== 'image' ) {
                        ...
                        if ( 'video' === $this.attr('data-type') ) {
                            this_item.html = '\
                            <div class="arunshah-pswp-media--video" data-src="' + $this.attr('href') + '">\
                            ...
                            this_item.html = '\
                            <div class="arunshah-pswp-media--iframe" data-player-src="'+ this_link +'" data-src="' + $this.attr('href') + '" data-type="' + this_type + '">\
                ...
                core.$dom.body.on('click', '.arunshah-pswp-image-wrap', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                ...
                jQuery(document).on('click', '.arunshah-lightbox-link', function(e) {
                    e.preventDefault();
                    if (typeof PhotoSwipe === 'function') {
                        let $this = jQuery(this),
                        ...
                        template.pswp.lightbox.listen('resize', function() {
                            if ( jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                            if ( jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                        });
                        ...
                        template.pswp.lightbox.listen('beforeChange', function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--video').each(function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.arunshah-pswp-media--iframe').each(function() {
                ...
                console.error('arunshah Error: PSWP script not found.');
            }
        }
    }
    ...
    function HeroSections() {
        // Masked Blocks
        if ( jQuery('.arunshah-masked-block').length ) {
            jQuery('.arunshah-masked-block').each(function() {
                new arunshah_Masked(this);
            });
        }
    }
    ...
    function BlocksHighlight() {
        if ( jQuery('.arunshah-backlight').length ) {
            template.cursor_pos = {
                x: 0,
                y: 0
            }
            jQuery(document).on('mousemove', function(e) {
                ...
                jQuery('.arunshah-backlight').css({
                    '--mouse-x': `${template.cursor_pos.x}px`,
                    '--mouse-y': `${template.cursor_pos.y}px`,
                });
            });

            if ( jQuery('.arunshah-block').length ) {
                jQuery('.arunshah-block').each(function() {
    ...
    // --- MARQUEE ELEMENT --- //
    function Marquee() {
        if ( jQuery('.arunshah-marquee').length ) {
            jQuery('.arunshah-marquee').each(function() {
                new arunshah_Marquee(this);
            });
        }
    }
        ...
        if ( jQuery('.swiper').length ) {
            ...
            const init_swiper = function() {
                ...
                jQuery('.swiper').each(function() {
                    ...
                    if ( core.$dom.body.hasClass('arunshah-fullscreen-page') ) {
                        options.mousewheel = true;
                    }
                    // CAROUSEL
                    if ( $this.hasClass('arunshah-carousel') ) {
                    ...
                    // SLIDER
                    if ( $this.hasClass('arunshah-slider') ) {
                        ...
                        // Slider Content
                        if ( $this.find('.arunshah-slide-content').length ) {
                            ...
                            $this.children('.swiper-wrapper').children('.swiper-slide').each(function(i) {
                                ...
                                if ( $slide.children('.arunshah-slide-content').length ) {
                                    const $slide_content = $slide.children('.arunshah-slide-content');
                                    ...
                                    $media.wrap('<div class="arunshah-slider-media"/>');
                                    ...
                                    $slide.children('.arunshah-slider-media').css('clip-path', `url(#${(content_mask_id + i)})`);
                    ...
                    // Pagination
                    if ( $this.children('.arunshah-dots').length ) {
                        options.pagination = {
                            el: '.arunshah-dots',
                            dynamicBullets: true,
                    ...
                    // Slider Navigation
                    if ( $this.parent().children('.arunshah-slider-nav').length ) {
                        let $this_nav = $this.parent().children('.arunshah-slider-nav');
                        options.navigation = {
                            prevEl: $this_nav.children('.arunshah-slider-prev')[0],
                            nextEl: $this_nav.children('.arunshah-slider-next')[0],
                        }
            ...
            if ( typeof Swiper === 'function' ) {
                ...
                console.error('arunshah Error: Swiper Library not found.')
            }
        }
    }
    ...
    function CircleLinks() {
        if ( jQuery('.arunshah-circle-path-link:not(.is-init)').length ) {
            jQuery('.arunshah-circle-path-link:not(.is-init)').each(function() {
                let $this = jQuery(this),
    ...
    function LinksEvents() {
        jQuery('.arunshah-back-to-listing').on('click', function() {
            window.localStorage.setItem('arunshah_return_from', this.getAttribute('data-id'));
        });
        if ( jQuery('.arunshah-albums-listing').length && window.localStorage.getItem('arunshah_return_from') != false ) {
            jQuery(window).trigger( 'arunshah_set_album', window.localStorage.getItem('arunshah_return_from') );
            window.localStorage.setItem('arunshah_return_from', 0);
        } else {
            window.localStorage.setItem('arunshah_return_from', 0);
        }
    }
    ...
    function InfiniteList() {
        if ( jQuery('.arunshah-infinite-list-wrap:not(.is-init)').length ) {
            jQuery('.arunshah-infinite-list-wrap:not(.is-init)').each(function() {
                this.classList.add('is-init');
                core.updateRequired.push( new arunshah_Infinite_List(this) );
            })
        }
    }
    ...
    // --- COUNTERS --- //
    function Counters() {
        if ( jQuery('.arunshah-counter:not(.is-init)').length ) {
            jQuery('.arunshah-counter:not(.is-init)').each(function() {
                let $this = jQuery(this);
                this.classList.add('is-init');
                this.counter_count = function() {
                    let $counter = $this.children('.arunshah-counter-number');
    ...
    function ListWithPreview() {
        if ( jQuery('.arunshah-list-with-preview').length ) {
            jQuery('.arunshah-list-with-preview').each(function() {
                new arunshah_LWP(this);
            });
        }
    }
    ...
    function BeforeAfter() {
        if ( jQuery('.arunshah-before-after:not(.is-init)').length ) {
            jQuery('.arunshah-before-after:not(.is-init)').each(function() {
                jQuery(this).addClass('is-init');
                core.updateRequired.push( new arunshah_Before_After(jQuery(this)) );
            });
        }
    }
    ...
    // --- TOGGLES --- //
    function Toggles() {
        if ( jQuery('.arunshah-toggles-item:not(.is-init)').length ) {
            jQuery('.arunshah-toggles-item:not(.is-init)').each(function() {
                let $this = jQuery(this);
                $this.addClass('is-init');
                $this.children('.arunshah-toggles-item--title').append('<i class="arunshah-icon arunshah-icon-arrow-down"/>');
                $this.children('.arunshah-toggles-item--content').slideUp(1);
                $this.on('click', function() {
                    jQuery(this).toggleClass('is-active');
                    $this.children('.arunshah-toggles-item--content').slideToggle(300);
                });
                $this.on('click', '.arunshah-toggles-item--content', function(e) {
                    e.stopPropagation();
                    e.preventDefault();
                });
            });
    ...
    function ScrollTrigger() {
        ...
        if ( jQuery('.arunshah-inline-ribbon:not(.is-init)').length ) {
            jQuery('.arunshah-inline-ribbon:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Sticky Elements
        if ( jQuery('.arunshah-sticky-block:not(.is-init)').length ) {
            jQuery('.arunshah-sticky-block:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Expand on Scroll
        if ( jQuery('.arunshah-expand-on-scroll:not(.is-init)').length ) {
            jQuery('.arunshah-expand-on-scroll:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Text Mask Fill
        if ( jQuery('.arunshah-text-fill-mask:not(.is-init)').length ) {
            jQuery('.arunshah-text-fill-mask:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Parallax Media
        if ( jQuery('.arunshah-parallax-media:not(.is-init)').length ) {
            jQuery('.arunshah-parallax-media:not(.is-init)').each(function() {
                let $this = jQuery(this);
    ...
    function ContactForm() {
        if ( jQuery('.arunshah-contact-form').length ) {
            // The code related to the contact form is in the file: js/contact_form.js
            if ( typeof arunshah_Contact_Form === 'function' ) {
                arunshah_Contact_Form();
            } else {
                console.error('arunshah Error: Contact form script not found.')
            }
        }
    }
    ...
    function InitAnimation() {
        ...
        template.checkURL = function( el ) {
            ...
            if ( this_href.indexOf('javascript') === 0 ) {
                ...
                jQuery(arunshah_config.linksException).each(function() {
                    if ( $el.is(this) ) {
                        result = false;
                    }
                });
    ...
    function Layout() {
        ...
        if ( core.$dom.body.hasClass('arunshah-fullscreen-page') ) {
            ...
            core.$dom.main.removeClass('arunshah-min-content');
            ...
            if ( core.$dom.main.height() < minHeight ) {
                core.$dom.main.addClass('arunshah-min-content');
    ...
    $(document).ready(function() {
        // General UI
        RightClickProtection( '.arunshah-rcp-wrap' );
        MainMenuInit();
        CircleLinks();
